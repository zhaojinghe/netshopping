    ...
    <!--Begin 所在收货地区 Begin-->
        ...
        <span class="fr">
          <c:if test="${sessionScope.loginUser==null}">
            <span class="fl">你好，请<a href="${ctx}/login?action=toLogin"  style="color:#ff4e00;">登录</a>&nbsp;<a href="${ctx}/pre/register.jsp" style="color:#ff4e00;">免费注册</a>&nbsp;&nbsp;</span>
          </c:if>
          <c:if test="${sessionScope.loginUser!=null}">
            <span class="fl"><a href="${ctx}/userinfo?action=index">${sessionScope.loginUser.username}</a>&nbsp;|&nbsp;<a href="${ctx}/orderList?action=index&userId=${sessionScope.loginUser.id}">我的订单</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null && sessionScope.loginUser.type==1}">
            <span class="fl">|&nbsp;<a href="${ctx}/productList">后台管理</a>&nbsp;</span>
          </c:if>
           <c:if test="${sessionScope.loginUser!=null}">
             <span class="fl">|&nbsp;<a href="${ctx}/loginout" >注销</a></span>
          </c:if>
        </span>
  </div>
</div>
